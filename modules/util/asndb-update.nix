# based on https://pages.madhouse-project.org/algernon/infrastructure.org/services_iocaine_asndb_update
...
let
  cfg = config.services.maxmind-db-update;
  inherit (utils.systemdUtils.unitOptions) unitOption;
in
{
  options = {
    services.maxmind-db-update = lib.mkOption {
      default = { };
  ...
  config = lib.mkIf cfg.enable {
    age.secrets.maxmind-geoip-credentials.file = ../../secrets/maxmind-geoip-credentials.age;
    systemd.services."maxmind-db-update" = {
      ...
      serviceConfig =
        ...
        in
        {
          ...
          LoadCredential =
            let
              maxmind-credentials = config.age.secrets.maxmind-geoip-credentials.path;
            in
            [ "maxmind-credentials:${maxmind-credentials}" ];

          StateDirectory = attachedConfig.StateDirectory;
